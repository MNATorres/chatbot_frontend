...
- **`src/hooks/useChat.ts`** — consumes `useFetch` and owns all chat state (`messages`, `isLoading`, `error`). `sendMessage` optimistically appends the user `Message`, POSTs to `/ask` as `{ message }`, and appends the assistant reply from `{ answer }`. The `Message` shape (`id`/`role`/`timestamp`) is defined and exported here.
...
The `/ask` request/response contract (`{ message }` → `{ answer }`) is the coupling point with the separate backend service. Keep it in sync with the backend.
